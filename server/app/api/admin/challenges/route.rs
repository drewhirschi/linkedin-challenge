//! `POST /api/admin/challenges` — create a challenge with its scoring rules.
//! Everyone in the org participates automatically; there is no enrollment step.
...
#[nextrs::api(
    ...
    responses(
        (status = 200, description = "Challenge created", body = CreateChallengeResponse),
        (status = 400, description = "Missing name or bad dates", body = ApiError),
...
pub async fn post(
    ...
    Ok(Json(CreateChallengeResponse { id: comp.id }))
}
